...
let themeService = {
        ...
        if (themes.length > 0) {
            $.each(themes, function (index, value) {
                ...
                let themeHtml = "<div class='col-sm-3 mb-3'>" +
                    ...
                    "<div class='d-block'>" +
                    "<button type='button' class='btn btn-outline-danger w-100'  onclick='applyTheme(" + "\"" + value.name + "\"" + ")'" + " > Install</button> " +
                    "<a href='" + encodeURI("/admin/deletetheme/" + encodeURIComponent(value.name)) + "' class='btn btn-outline-danger w-100 mt-2'>Delete</a>" +
    ...
    updateTemplate: function () {
        ...
        connection.invoke("UpdateTemplate", templateType.value, templateContent.value).catch((error) => {
            alert(error);
    ...
    updateTheme: function () {
        connection.invoke("UpdateTheme", themeType.value, themeName.value, templateContent.value).catch((error) => {
            alert(error);
            $(".spinner").css("display", "none");
    ...
    getThemes: function () {
        connection.invoke("GetThemes").catch((error) => {
            alert(error);
            $(".spinner").css("display", "none");
    ...
    getTemplate: function () {
        connection.invoke("GetTemplate", templateType.value).catch((error) => {
            alert(error);
            $(".spinner").css("display", "none");
    ...
    deleteTheme: function () {
        connection.invoke("DeleteTheme", themeName.value).catch((error) => {
            alert(error);
            $(".spinner").css("display", "none");
...
function applyTheme(_themeName) {
    $(".spinner").css("display", "block");
    connection.invoke("Apply", _themeName).catch((error) => {
        alert(error);
        $(".spinner").css("display", "none");

    });
}
connection.on("applied", function (isApplied,error) {
    $(".spinner").css("display", "none");
    if (isApplied) {
      
        window.alert("Themes Installed");
    } else {
        window.alert(error);
    }
});
...
connection.on("update", function (isSaved,error) {
