...
let mediaService = {
        ...
        if (medias.length > 0) {
            $.each(medias, function (index, media) {
                ...
                let container = "<div class='media shadow-sm align-content-center pt-2 postBody mt-3'>" +
                    ...
                    "<p class='main-title'style='font-size:18px;'" + "  onclick = 'copyUrl(" + "\"" + url + media.fileName + "\"" + ")'" + " >" + media.name + "</p>" +
                    " </a>" +
                    ...
                    "</div>" +
                    "</div>" +
                    "<div class='main-drop ml-auto' style='cursor: pointer'  Title='Click on the file name to copy'> " +
                    "<i class='fa fa-copy text-success main-drop-icon'" + "  onclick = 'copyUrl(" + "\"" + url + media.fileName + "\"" + ")'" +" ></i>" +
                    "</div>" +
    ...
    getFileByCategory: function (category) {
        connection.invoke("GetFileByCategory", category).catch((error) => {
            alert(error);
    ...
    getFileByName: function (name, category) {
        connection.invoke("GetFileByName", name, category).catch((error) => {
            alert(error);
            $(".spinner").css("display", "none");
    ...
    deleteMedia: function (url) {
        connection.invoke("DeleteFile", url).catch((error) => {
            alert(error);
            $(".spinner").css("display", "none");
    ...
    deleteMedias: function (urls) {
        connection.invoke("DeleteFiles", urls).catch((error) => {
            alert(error);
            $(".spinner").css("display", "none");
...
function copyCallback(text) {
    ...
    try {
        ...
        var successful = msg ? 'File Copied' : 'File not Copied error occur';
        alert(successful);
    }
    catch (error) {
        console.error(error);
            ...
            if (checkbox.checked) {
                ...
                $(".postBody").each(function (index, htmlElement) {
                    ...
                    postCheckbox.addEventListener("click", function () {
                        ...
                        let mediaIndex = mediaUrls.indexOf(media);
