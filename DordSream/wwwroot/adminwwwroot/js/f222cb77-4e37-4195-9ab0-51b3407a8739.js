...
let postService = {
        ...
        if (posts.length > 0) {
            $.each(posts, function (index, post) {
                ...
                let container = "<div class='media shadow-sm align-content-center pt-2 postBody mt-3'>" +
                    ...
                    "<a href='https://www.facebook.com/sharer.php?u=" + url + encodeURI(encodeURIComponent(post.name) + "/" + encodeURIComponent(post.titleLink) + "/" + encodeURIComponent(post.postId)) + "' class='dropdown-item-text media'>" +
                    "<i class='fa fa-facebook mr-2'></i>" +
                    "<p>Facebook</p>" +
                    "</a>" +
                    "</li>" +
                    "<li class='dropdown-item'>" +
                    "<a href='https://plus.google.com/share?url=" + url + encodeURI(encodeURIComponent(post.name) + "/" + encodeURIComponent(post.titleLink) + "/" + encodeURIComponent(post.postId)) + "' class='dropdown-item-text media'>" +
                    "<i class='fa fa-google-plus mr-2'></i>" +
                    "<p>Google PLus</p>" +
                    "</a>" +
                    "</li>" +
                    "<li class='dropdown-item'>" +
                    "<a href='https://twitter.com/intent/tweet?url=" + url + encodeURI(encodeURIComponent(post.name) + "/" + encodeURIComponent(post.titleLink) + "/" + encodeURIComponent(post.postId))+ "' class='dropdown-item-text media'>" +
                    ...
                    "</ul>" +
                    "</div>" +
                    "<a href=" + encodeURI("/" + encodeURIComponent(post.name) + "/" + encodeURIComponent(post.titleLink) + "/" + encodeURIComponent(post.postId)) +"    target ='_blank'  "+ " class='media ml-2 justify-content-center align-items-baseline'>" +
                    "<p>" + post.comments + "</p>" +
    ...
    publish: function () {
        connection.invoke("Publish", title.value, content.value, label.value);
     
    },
    //Save Post From Server
    save: function () {
       
        connection.invoke("Save", title.value, content.value, label.value);
     
    },
    //Update publish Post
    updatePublish: function () {
        connection.invoke("UpdatePublish", id.value, title.value, content.value, label.value);
      
    },
    // Update Save Post From Server
    updateSave: function () {
        connection.invoke("UpdateSave", id.value, title.value, content.value, label.value);
      
    },
   
    // Get Published Post From Server
    getPublishPost: function () {
        connection.invoke("GetPublishPosts");
    },
    // Get Draft Post From Server
    getDraftPost: function () {
        connection.invoke("GetDraftPosts");
    },
    //Get All Posts From Server
    getAllPosts: function () {
        connection.invoke("GetAllPosts");
    },
    // Get Posts By Date From Server
    getPostByDate: function () {
        connection.invoke("GetPostsByDate", dateSearch.value);
    },
    //Get Post By Title
    getPostByTitle: function (title) {
       
      connection.invoke("GetPostsByTitle", title);
    },
    // Delete All Post From Server
    deleteAll: function () {
        if (postIds.length > 0) {

            connection.invoke("DeleteAll", postIds);
        }
        else {
    ...
    unpublishAll: function () {
        if (postIds.length > 0) {
            ...
            connection.invoke("UnpublishPosts", postIds);
        }
        else {
            alert("No Post Selected");
            $(".spinner").css("display", "none");
    ...
    applyLabels: function () {
        if (postIds.length > 0 && labelsInput.value != '') {
            connection.invoke("ApplyLabels", postIds, labelsInput.value);
        } else {
    ...
    deletePost: function (postId) {
        if (postId != '') {
            ...
            connection.invoke("Delete", postId);
        }
        else {
    ...
    applyLabel: function (postId) {
        if (postId != '') {
            ...
            connection.invoke("ApplyLabel", postId, labelInput.value);
        }
        else {
            alert("Post Id  Cannot be Empty");
            $(".spinner").css("display", "none");
    ...
    unpublish: function (postId) {
        if (postId != '') {
            ...
            connection.invoke("UnpublishPost", postId);
        }
        else {
            alert("Post Id  Cannot be Empty");
            $(".spinner").css("display", "none");
