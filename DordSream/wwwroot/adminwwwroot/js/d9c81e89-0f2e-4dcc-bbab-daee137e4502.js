...
let DbConnection = function () {
    ...
    DbConnection.prototype.context = new DbContext(hubConnection);


};


let DbContext = function (hubConnection) {

    DbContext.prototype.result = function (_callback) {
        hubConnection.on("result", function (result) {
            _callback(result.done, result.error, result.tableName);

        });
    };
    DbContext.prototype.reader = function (_callback) {
        hubConnection.on("reader", function (reader) {
            _callback(reader.result, reader.error, reader.tableName);
        });
    };
    ...
    DbContext.prototype.executeQuery = function () {
        ...
        if (typeof (_callback) != "function" && typeof (_callback) == "string") {
            if (_callback.includes(">>")) {
                let tableName = _callback.replace(">>", "");
                hubConnection.invoke("ExecuteMultipleQuery", query, values, tableName);
            }
        }
        else {
    ...
    DbContext.prototype.executeQueryReader = function () {
        ...
        if (typeof (_callback) != "function" && typeof (_callback) == "string") {
            if (_callback.includes(">>")) {
                let tableName = _callback.replace(">>", "");
                hubConnection.invoke("ExecuteMultipleQueryReader", query, values, tableName);
